...
trait HasPermissions
{
    ...
    public static function bootHasPermissions()
    {
        static::deleting(function ($model) {
            ...
            // Only call users() if it exists on the model
            if (is_a($model, Role::class) && method_exists($model, 'users')) {
                $model->users()->detach();
            }
    ...
    private function collectPermissions(...$permissions): array
    {
        return collect($permissions)
            ...
            ->reduce(function ($array, $permission) {
                ...
                $permission = $this->getStoredPermission($permission);                if (! $permission instanceof Permission) {
                    return $array;
                }

                if (! in_array($permission->id, $array)) {
