...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const exportLinks = document.querySelectorAll('.dropdown-item[href^="{{ route("employees.export") }}"], .dropdown-item[href^="{{ route("users.export") }}"]');
    ...
    function exportTableToPDF(tableID, filename) {
        ...
        doc.autoTable({
            ...
            // Customize the columns to exclude action buttons and format images
            didParseCell: function(data) {
                ...
                // Simplify the User column to just show the name
                if (data.column.index === 0 && data.section === 'body') {
                    // Extract just the user name without the date
                    const text = data.cell.text.toString();
                    const nameMatch = text.split(/\s+\d{4}/).shift();
                    if (nameMatch) {
                        data.cell.text = nameMatch.trim();
                    }
                }
            }
        });
